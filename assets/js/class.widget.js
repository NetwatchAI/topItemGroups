...
class CWidgetTopItemGroups extends CWidget {
	...
	#makeUrls() {
		...
		for (const button of this.#table_body.querySelectorAll('.js-show-binary')) {
			...
			curl.setArgument('action', 'widget.topitemgroups.value.check');
			...
			return new Promise((resolve, reject) => {
				...
				.then(binary_data => {
					...
					switch (binary_data.type) {
						case CWidgetTopItemGroups.VALUE_TYPE_IMAGE:
						...
						case CWidgetTopItemGroups.VALUE_TYPE_RAW:
							this.#makeBinaryButton(buttons, binary_data);
							break;
		...
		for (const button of buttons) {
			...
			switch (binary_data.type) {
				...
				case CWidgetTopItemGroups.VALUE_TYPE_RAW:
					...
					curl.setArgument('action', 'widget.topitemgroups.binary_value.get');
	...
	#addHintbox(button, content, curl = null) {
		...
		if (content instanceof HTMLImageElement) {
			button.addEventListener('onShowHint.hintBox', e => {
				...
				curl.setArgument('action', 'widget.topitemgroups.image_value.get');
