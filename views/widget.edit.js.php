...
window.widget_form = new class extends CWidgetForm {
	...
	init({templateid}) {
		...
		this.#form.querySelector('[name="groupby_mode"]').addEventListener('change', () => this.#updateForm());
		this.#updateForm();
	...
	/**
	 * Show only the group-by fields relevant to the selected mode.
	 */
	#updateForm() {
		const mode = this.#form.querySelector('[name="groupby_mode"]').value;

		const show_pattern = [
			<?= GroupKeyResolver::MODE_ITEM_NAME_PATTERN ?>,
			<?= GroupKeyResolver::MODE_ITEM_KEY_PATTERN ?>,
			<?= GroupKeyResolver::MODE_REGEX ?>
		].includes(parseInt(mode));

		const show_key_param = parseInt(mode) === <?= GroupKeyResolver::MODE_ITEM_KEY_PARAMETER ?>;
		const show_tag = parseInt(mode) === <?= GroupKeyResolver::MODE_ITEM_TAG ?>;
		const show_match_field = parseInt(mode) === <?= GroupKeyResolver::MODE_REGEX ?>;

		this.#toggleRow('js-groupby-pattern-row', show_pattern);
		this.#toggleRow('js-groupby-capture-row', show_pattern);
		this.#toggleRow('js-groupby-key-param-row', show_key_param);
		this.#toggleRow('js-groupby-tag-row', show_tag);
		this.#toggleRow('js-groupby-match-field-row', show_match_field);
	}

	#toggleRow(row_class, visible) {
		for (const element of this.#form.querySelectorAll(`.${row_class}`)) {
	...
	#processColumnsAction(e) {
		...
		switch (target.getAttribute('name')) {
			case 'add':
				...
				column_popup = PopUp(
					'widget.topitemgroups.column.edit',
					{
					...
					{
						dialogueid: 'topitemgroups-column-edit-overlay',
