...
if ($data['error'] !== null) {
	...
	foreach ($data['rows'] as ['columns' => $columns, 'context' => $context]) {
			...
			switch ($column_config['data']) {
				...
				case CWidgetFieldColumnsList::DATA_GROUP_NAME:
					$row[] = (new CCol($column['value']))
						->setAttribute('bgcolor', $color !== '' ? '#'.$color : null)
						->setAttribute('title', $column['value']);
		...
		if (!$is_empty_row) {
			$table_row = (new CRow($row))->setAttribute('data-group', $context['group']);

			// A merged row (context has no 'hostid') has no single correct host to attribute it to, so it is left
			// unselectable/unbroadcastable rather than pointing at an arbitrary contributing host - both
			// CWidgetTopItemGroups's selection and broadcast logic already handle a row with no data-hostid safely
			// (REPORT.md Q14). Every non-merged row still behaves exactly as it did in Top hosts.
			if (array_key_exists('hostid', $context)) {
				$table_row->setAttribute('data-hostid', $context['hostid']);
			}
